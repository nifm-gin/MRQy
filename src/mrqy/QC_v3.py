...
class BaseVolume_nondicom(dict):
    ...
    def __init__(self, fname_outdir, v, ol, scan, sample_size, ch_flag, volume_masks=None, Patient_name=''):
        # Initialize the dictionary
        dict.__init__(self)
        ...
        self.addToPrintList("Name of Subject", Patient_name, v, ol, 24)
        self["outdir"] = fname_outdir
...
if __name__ == '__main__':
    ...
    dicom_flag, nondicom_flag, mat_flag = False, False, False
    ...
    if nondicom_flag:
        for l,k in enumerate(nondicom_spli):
            try:
                ...
                path_name = Path(patients[0])
                patient_name = path_name.parent.name
                ...
                s = BaseVolume_nondicom(fname_outdir, v, l + 1, k, sample_size, ch_flag, volume_masks, patient_name)
                worker_callback(s, fname_outdir)
            except Exception as e:
