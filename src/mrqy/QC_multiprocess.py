...
from pathlib import Path  # Plus rapide que os.path
...
class BaseVolume_nondicom(dict):
    ...
    def __init__(self, fname_outdir, v, ol, scan, sample_size, ch_flag, volume_masks=None):
        # Initialize the dictionary
        dict.__init__(self)
        ...
        if volume_masks is not None:
            voxel_spacing = v[2].get_voxel_spacing()
            roi_volume = calculate_roi_volume(v[0], volume_masks[0], voxel_spacing)
            self.addToPrintList("ROIVol", roi_volume, v, ol, 23)  # Ajoutez ROIVol à l'index 23
        else:
            self.addToPrintList("ROIVol", "N/A", v, ol, 23)  # Cas sans masque
...
def calculate_roi_volume(volume_slices, mask_slices, voxel_spacing):
    """
    Calcule le volume total du ROI en mm³.
    Args:
        volume_slices: Liste des slices 2D du volume (ex: v[0] pour NIFTI)
        mask_slices:   Liste des slices 2D du masque (ex: volume_masks[0])
        voxel_spacing: Tuple (VRX, VRY, VRZ) en mm (ex: v[2].get_voxel_spacing())
    Returns:
        Volume du ROI en mm³ (float)
    """
    mask_3d = np.stack(mask_slices)  # Convertit la liste de slices en volume 3D
    roi_voxels = np.sum(mask_3d > 0)  # Compte les voxels du ROI (masque > 0)
    voxel_volume = voxel_spacing[0] * voxel_spacing[1] * voxel_spacing[2]  # Volume d'un voxel
    return roi_voxels * voxel_volume  # Volume total du ROI
...
def process_nifti_file(args):
    """Traite un fichier NIFTI en parallèle."""
    k, nondicom_name, l, fname_outdir, output_masks, sample_size, ch_flag = args
    output_dir = Path(fname_outdir) / nondicom_name
    if output_dir.exists():
        print(f"Skipping {nondicom_name}: Already processed.")
        return None
    try:
        mask = brain_extraction(k, output_masks)
        if mask is None:
            print(f"Skipping {k}: Brain extraction failed.")
            return None
        v = volume_nifti(k, nondicom_name)
        volume_masks = volume_nifti_masks(mask)
        saveThumbnails_nondicom(v, fname_outdir, volume_masks[0])
        s = BaseVolume_nondicom(fname_outdir, v, l + 1, k, sample_size, ch_flag, volume_masks)
        return s
    except Exception as e:
        print(f"Error processing {nondicom_name}: {e}")
        return None
...
if __name__ == '__main__':
    ...
    if nondicom_flag:
        ...
        file_args = [
            (k, nondicom_names[l], l, fname_outdir, output_masks, sample_size, ch_flag)
            for l, k in enumerate(nondicom_spli)
        ]
    
        # Lancer le traitement en parallèle (1 processus = 1 fichier)
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = [executor.submit(process_nifti_file, args) for args in file_args]
